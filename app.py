...
def analyser_article(row, marque):
    try:
        texte = row['contenu']
        ...
        sentiment = "Neutral"
    modele = detecter_modele(row['titre'], marque)
...
if st.button("🔍 Lancer la veille"):
    ...
    if not articles.empty:
        with st.spinner("Analyse des articles..."):
            articles[['résumé', 'ton', 'modèle']] = articles.apply(analyser_article, axis=1, marque=filtre_marque)
